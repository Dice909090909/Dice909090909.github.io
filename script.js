...
function handleFindElement() {
    const symbolInput = document.getElementById('element-symbol');
    ...
    if (element) {
        ...
        resultDiv.textContent = `Element with symbol '${symbol}' not found in the periodic table.`;
        resultDiv.classList.add('error');
    }
}

document.getElementById('find-element').addEventListener('click', handleFindElement);

document.getElementById('element-symbol').addEventListener('keypress', function(event) {
    if (event.key === 'Enter') {
        handleFindElement();
    }
});
